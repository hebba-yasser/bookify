...
class _SignInViewBodyState extends State<SignInViewBody> {
  ...
  bool hasNavigated = false; // Prevents duplicate navigation
    ...
    return BlocListener<AuthCubit, AuthState>(
      listener: (context, state) async {
        if (state is AuthLoginSuccess) {
          ...
          await context.read<UserDataCubit>().fetchUserData(id: uId ?? '');

          /// Navigate only once to HomeView after user data is loaded
          if (!hasNavigated) {
            hasNavigated = true;
            Future.delayed(Duration(milliseconds: 300), () {
              navigatorPush(context, const HomeView());
            });
          }
        }
